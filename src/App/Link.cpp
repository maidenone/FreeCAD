                ...
                if(elementCount>elementList->getSize()) {
                    ...
                    for(;i<elementCount;++i) {
                        ...
                        if((int)placementList.size()>i) {
                            ...
                            Base::Placement pla(Base::Vector3d(i*5,0,0),Base::Rotation());
...
#define GET_LINK_ELEMENT(_i,_list, _obj) \
    ...
    }else if(prop == getSubNameProperty()) {
        mySub = getSubNameValue();
        if(mySub.size() && mySub[mySub.size()-1]!='.')
            mySub += '.';
    }else if(prop == getLinkedObjectProperty()) {
        if(!prop->testStatus(Property::User3) && getLink()) {
            DocumentObject *obj = 0;
            extensionGetLinkedObject(obj,false,0,false,0);
            if(!obj) {
                // auto clear Sub if linked object changed
                getSubNameProperty()->setValue("");
            }
        }
    }
}
        ...
        const std::vector<std::string> &subElements) 
{
    ...
    auto subProp = getSubNameProperty();
    ...
    if(obj) {
        ...
        if(!linkProp->isDerivedFrom(PropertyXLink::getClassTypeId())) {
            auto parent = getContainer();
    ...
    if(subname && subname[0]) {
        ...
        // setStatus to reduce unnecessary view provider update
        subProp->setStatus(Property::User3,true);
        subProp->setValue(subname);
        subProp->setStatus(Property::User3,false);
    }
