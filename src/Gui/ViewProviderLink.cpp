...
ViewProviderLink::ViewProviderLink()
    :prevLinked(0),linkType(LinkTypeNone),linkTransform(false)
{
...
void ViewProviderLink::onChangeIcon() const {
    ...
    if(hasElements(ext)) {
        ...
        for(int i=0;i<(int)elements.size();++i) {
            auto vp = dynamic_cast<ViewProviderLinkElement*>(
                    Application::Instance->getViewProvider(elements[i]));
            if(vp) {
                if(!vp->myOwner) {
                    vp->myOwner = const_cast<ViewProviderLink*>(this);
                    vp->myIndex = i;
                }
                vp->signalChangeIcon();
            }
...
QIcon ViewProviderLink::getIcon() const {
    QIcon icon;
    auto ext = getLinkExtension();
    if(ext && ext->getElementCountValue() && hasElements(ext)) 
...
void ViewProviderLink::updateDataPrivate(App::LinkBaseExtension *ext, const App::Property *prop) {
    ...
    if(prop == &ext->_LinkRecomputed) {
        if(handle.hasSubs())
            handle.onLinkUpdate();
        auto linked = getObject()->getLinkedObject(false);
        if(prevLinked && linked!=prevLinked)
            signalChangeIcon();
        prevLinked = linked;
             ...
             prop == ext->getSubElementsProperty()) 
    {
        ...
        if(!hasElements(ext)) {
            ...
            for(int i=0;i<propElements->getSize();++i) {
                auto obj = dynamic_cast<App::LinkElement*>((*propElements)[i]);
                if(!obj) continue;
                auto vpe = dynamic_cast<ViewProviderLinkElement*>(
        ...
        if (prop == &ShapeMaterial) {
            if(myOwner) {
                if(!ShapeMaterial.testStatus(App::Property::User3)) {
                    ...
                    if(myOwner->UseMaterialList.getSize()<=myIndex) 
                        myOwner->UseMaterialList.setSize(myIndex+1,false);
                    myOwner->UseMaterialList.set1Value(myIndex,true,false);
                    UseMaterial.setStatus(App::Property::User3,false);

                    myOwner->MaterialList.setStatus(App::Property::User3,true);
                    if(myOwner->MaterialList.getSize()<=myIndex) 
                        myOwner->MaterialList.setSize(myIndex+1,App::Material(App::Material::DEFAULT));
                    myOwner->MaterialList.set1Value(myIndex,ShapeMaterial.getValue(),true);
                    myOwner->MaterialList.setStatus(App::Property::User3,false);

                    if(!myOwner->LinkUseMaterial.getValue())
                        myOwner->LinkUseMaterial.setValue(true);
                }
            }
        }else if(prop == &UseMaterial) {
            if(myOwner) {
                if(!UseMaterial.testStatus(App::Property::User3)) {
                    myOwner->UseMaterialList.setStatus(App::Property::User3,true);
                    if(UseMaterial.getValue()) {
                        if(myOwner->UseMaterialList.getSize()>myIndex) 
                            myOwner->UseMaterialList.set1Value(myIndex,true,true);
                    }else {
                        if(myOwner->UseMaterialList.getSize()<=myIndex)
                            myOwner->UseMaterialList.setSize(myIndex+1);
                        myOwner->UseMaterialList.set1Value(myIndex,false,true);
                    }
                    myOwner->UseMaterialList.setStatus(App::Property::User3,false);

                    if(UseMaterial.getValue() && !myOwner->LinkUseMaterial.getValue())
                        myOwner->LinkUseMaterial.setValue(true);
                }
            }
        }
...
QIcon ViewProviderLinkElement::getIcon(void) const {
    ...
    FC_LOG(name << " change icon");
    if(myOwner) {
        QIcon icon = myOwner->getLinkedIcon();
        if(!icon.isNull()) return icon;
    }
    char pxname[64];
    snprintf(pxname,sizeof(pxname),"LinkElement%d",myIndex<0?0:myIndex%3);
    return Gui::BitmapFactory().pixmap(pxname);
}

std::vector<App::DocumentObject*> ViewProviderLinkElement::claimChildren(void) const {
    if(myOwner)
        return myOwner->childrenCache;
    return std::vector<App::DocumentObject*>();
}
////////////////////////////////////////////////////////////////////////////////////////
